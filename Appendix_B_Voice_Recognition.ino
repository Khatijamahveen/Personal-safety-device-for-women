...
void setup()
{
  ...
  pinMode(HELP_OUT_PIN, OUTPUT); // Set the pin as output
  digitalWrite(HELP_OUT_PIN, HIGH); // Turn off
...
void loop()
{
  ...
  // ===== HELP DETECTED ACTION =====
  if (pred_index == 0 && pred_value > 0.90)
  {
    Serial.println("HELP DETECTED!");

    // LED ON (LOW)
    digitalWrite(HELP_OUT_PIN, LOW);
    digitalWrite(LED_BUILTIN, LOW);
    Serial.println("LED ON");
    delay(5000); // keep LED ON for 5 seconds

    // LED OFF (HIGH)
    digitalWrite(HELP_OUT_PIN, HIGH);
 ...
 *
 * @param[in] n_samples The n_samples
 *
