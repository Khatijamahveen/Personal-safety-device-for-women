...
#define SAFE_BUTTON 14  // Safe confirmation button
#define HELP_BUTTON 12  // HELP button (active LOW)

bool sosActive = false;
unsigned long lastAlertTime = 0;
const unsigned long ALERT_INTERVAL = 15000; // 15 seconds
...
void setup()
{
  ...
  pinMode(SAFE_BUTTON, INPUT_PULLUP); // IMPORTANT (was missing)
...
void loop()
{
  ...
  int helpButtonState = digitalRead(HELP_BUTTON);

  // ---------- SOS LOGIC ----------
  if (sensorData == LOW && !sosActive && millis() - lastAlertTime > ALERT_INTERVAL)
  {
    sosActive = true;
    lastAlertTime = millis();
    digitalWrite(4, HIGH); // LED ON
    sendCapturedImage2Telegram(myToken, myChatId);
    sendMessage2Telegram(
      myToken,
      myChatId,
  ...
  if (safeButtonState == LOW && lastSafeButtonState == HIGH)
  {
    sosActive = false;
    digitalWrite(4, LOW);
    sendMessage2Telegram(
  ...
  // -------- HELP BUTTON --------
  if (helpButtonState == LOW && lastHelpButtonState == HIGH)
  {
    sosActive = true;
    lastAlertTime = millis();
    digitalWrite(4, HIGH);
  ...
  lastHelpButtonState = helpButtonState;
